...
\begin{document}
	...
	Sabendo que a carga está distribuída de maneira uniforme por todo o fio, temos que:
	
	\[
	\lambda = \frac{dQ}{dz}\Rightarrow dQ=\lambda dz
	\]
	
	Temos que a força $d\textbf{F}$ sentida pela carga $q$ é:
	
	\[
	d\textbf{F}=-k\frac{qdQ}{r^2}\textbf{r}
	\]
	
	\[
	d\textbf{F}=-k\frac{q\lambda dz}{r^2}\left(\sin\theta\textbf{i}+\cos\theta\textbf{j}\right)
	\]
	
	Ou seja,
	
	\[
	\textbf{F}=\int_{min}^{max}\]
